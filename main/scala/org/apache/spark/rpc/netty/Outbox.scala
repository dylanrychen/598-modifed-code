...
private[netty] case class OneWayOutboxMessage(content: ByteBuffer) extends OutboxMessage
  ...
  override def sendWith(client: TransportClient): Unit = {
    ...
    // logWarning(s"Client is " + client + ", sender's IP is " + localIpAddress +
    // ", content is " + content)
    // var log: String = ("Client:" + client + "*Sender:" + localIpAddress +
    // "*Content" + content)
    // log = log.replace(" ", "%20")
    // log = log.replace("[", "\\[")
    // log = log.replace("]", "\\]")
  ...
  override def getcontent(): ByteBuffer = {
    content
  }
...
private[netty] case class RpcOutboxMessage(
  ...
  override def sendWith(client: TransportClient): Unit = {
    ...
    // var log: String = ("Client:" + client + "*Sender:" + localIpAddress +
    // "*Content" + content)
    // // val response: HttpResponse[String] = Http("http://foo.com/search").param("log",log).asString
    // log = log.replace(" ", "%20")
    // log = log.replace("[", "\\[")
    // log = log.replace("]", "\\]")
    // var curl: String = "curl 10.193.205.231:80/?log=" + log
    ...
    // logWarning(s"curr is " + curl)
    // logWarning(s"begin to send")
    // val result = Process(curl).run()
    // logWarning(s"ended")
